...
module.exports = {
  insert, index, update, destroy, detail, getRelativeBlogs, tagCloud
}
...
async function getRelativeBlogs (id, tag) {
  let queryParam = { "tags": { $all: [tag] }, "_id": { $ne: id }, "createdAt": { $lte: new Date() } }
...
async function tagCloud () {
  ...
  await Promise.all(
    tag_array.map(async (tag_val) => {
      ...
      let tmp = {}
      let blogCount = await Blog.find({ "tags": { $all: [tag_val.key] }, "createdAt": { $lte: new Date() } }).countDocuments()
      tmp.key = tag_val.key
      tmp.num = blogCount
      tag_tmp.push(tmp)
    })
